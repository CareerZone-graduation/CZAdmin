...
import { CheckCircle, XCircle, Building2, Calendar, Info, RotateCcw, Bot, Search, CalendarDays, CalendarClock } from 'lucide-react';
...
export function AIModeratedJobs() {
  ...
  const filterJobsByDate = (jobsList) => {
    ...
    if (dateFilter === 'custom' && customDate) {
      ...
      return jobsList.filter(job => {
        const moderatedDate = new Date(job.aiModerationResult?.moderatedAt);
        return moderatedDate >= startOfDay && moderatedDate < endOfDay;
      });
    }
    
    const range = getDateRange(dateFilter);
    if (!range) return jobsList;

    return jobsList.filter(job => {
      const moderatedDate = new Date(job.aiModerationResult?.moderatedAt);
      return moderatedDate >= range.from && moderatedDate <= range.to;
    });
  };
  ...
  const fetchAIModeratedJobs = useCallback(async (page = meta.currentPage) => {
    try {
      ...
      // Apply date filter
      const filteredJobs = filterJobsByDate(aiModeratedJobs);

      setJobs(filteredJobs);
      setMeta({
        ...response.data.meta,
        totalItems: filteredJobs.length,
        totalPages: Math.ceil(filteredJobs.length / meta.limit)
      });
    } catch (error) {
  ...
  }, [searchTerm, statusFilter, dateFilter, meta.limit]);
  ...
  const handleQuickUndo = async (filter, customDateValue = null) => {
    let jobsInRange;
    let filterName;
    ...
    if (filter === 'custom' && customDateValue) {
      ...
      filterName = `ngày ${selectedDate.toLocaleDateString('vi-VN')}`;
    } else {
    ...
    if (jobsInRange.length === 0) {
      toast.info('Không có job nào trong khoảng thời gian này');
      return;
    }
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <Select value={dateFilter} onValueChange={(value) => {
              setDateFilter(value);
              setShowCustomDateInput(value === 'custom');
            }}>
              <SelectTrigger>
              ...
              <SelectContent>
                ...
                <SelectItem value="last7days">7 ngày gần nhất</SelectItem>
                <SelectItem value="last30days">30 ngày gần nhất</SelectItem>
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => handleQuickUndo('last7days')}
              disabled={loading}
              className="text-orange-600 border-orange-300 hover:bg-orange-50"
            >
              <CalendarDays className="w-4 h-4 mr-1" />
            ...
            <Button
              size="sm"
              variant="outline"
              onClick={() => handleQuickUndo('last30days')}
              disabled={loading}
              className="text-orange-600 border-orange-300 hover:bg-orange-50"
            >
              <CalendarDays className="w-4 h-4 mr-1" />
            ...
            {customDate && (
              <Button
                size="sm"
                variant="outline"
                onClick={() => handleQuickUndo('custom', customDate)}
                disabled={loading}
                className="text-orange-600 border-orange-300 hover:bg-orange-50"
              >
                <CalendarClock className="w-4 h-4 mr-1" />
                Hoàn tác ngày {new Date(customDate).toLocaleDateString('vi-VN')}
              </Button>
            )}
          </div>
